...
const enableCover = (job) => {
  ...
  const staticCover = document.querySelector("#static-cover");
  console.log(activeCover, staticCover);
  activeCover
    ? hideElement(activeCover, false)
    : hideElement(staticCover, false);
};
...
const createSliderItem = (job, templateItem, wrapper) => {
  ...
  applyButton.setAttribute(
    "href",
    `https://www.${focusPageRootUrl}?id=${job.id}`
  );
